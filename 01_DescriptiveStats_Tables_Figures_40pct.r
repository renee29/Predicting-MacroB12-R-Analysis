...
output_table1_dir <- file.path(script_dir, "R_Outputs_Table1_40pct")
if (!dir.exists(output_table1_dir)) { dir.create(output_table1_dir) } else { cat("Table 1 output directory exists:", normalizePath(output_table1_dir), "\n") }

# --- Constantes ---
RANDOM_STATE <- 42; set.seed(RANDOM_STATE)
IMPUTATION_TO_USE <- 1
MACROB12_THRESHOLD <- 40
...
# =======================================================================
cat("\n--- X. Saving Table 1 Numerical Results to CSV ---\n")
...
table1_csv_path <- file.path(output_table1_dir, "Table1_Descriptive_Statistics_Numerical.csv")
tryCatch({
...
cat(sprintf("Output saved in '%s'.\n", normalizePath(output_table1_dir)))
